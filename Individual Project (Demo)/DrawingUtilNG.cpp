...
void DrawingUtilNG::drawRectangle(double x, double y, int sizeX, int sizeY, bool filled, double angle)
{
	// perform transformation
	glLoadIdentity();									// Reset the current matrix to default
	glTranslatef(x, y, 0);								// Move to x, y
	glRotatef(angle, 0.0f, 0.0f, -1.0f);			    // Rotate about the -z axis (y is down, but I want angle CCW)
	glScalef(2.f, 2.f, 2.f);

	// draw the rectangle "normally" in its local coordinate system
	if (filled)
		glBegin(GL_POLYGON);
	else
		glBegin(GL_LINE_LOOP);
	...
	// Reset the current  matrix to default 
	glLoadIdentity();									
}
