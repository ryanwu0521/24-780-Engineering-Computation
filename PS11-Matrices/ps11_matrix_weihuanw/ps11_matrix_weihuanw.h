/*
Ryan Wu Based on Dr.Nestor Gomez's Work
Carnegie Mellon University
24-780-B Engineering Computation
PS 11: Matrices
Program Description: This program can perform matrix multiplication, transpose, and determinant.
The is the header file for the Matrix2D class with all it's functions
Due: Wed, Nov. 30, 2022
...
private:
	...
	T* content;  // will use NC to get the 2D location
...
inline Matrix2D<T, NR, NC>& Matrix2D<T, NR, NC>::operator=(const Matrix2D<T, NR, NC>& original) {
	...
	//// do I need to clear old stuff? (no, because it's the same size)
	//if (content != nullptr)
	//	delete[] content;
...
{
	if (NR != NC)
		throw std::out_of_range("Cannot transpose a non-square matrix");
	else {
		for (int i = 0; i <= NR; i++)
			for (int j = i + 1; j < NC; j++)
				std::swap(content[i * NC + j], content[j * NC + i]);
...
inline void Matrix2D<T, NR, NC>::matrixMultiplyParallel(Matrix2D<T, NC, NR>& otherMatrix, Matrix2D<T, NR, NR>& resultMatrix)
{
	...
	if (NC < multiThreadCutoff)
		...
		std::vector<std::thread> theThread;
		theThread.resize(numbThreads);


		/*for (int i = 0; i < numbThreads; i++) {
			std::thread aThread(Matrix2D<T, NR, NC>::threadEntry, this);
			theThread[i].swap(aThread);
			currThread++;
		}*/

		int lastTread;
		for (int i = 0; i < numbThreads; i++) {
			lastTread = currThread;
			std::thread aThread(Matrix2D<T, NR, NC>::threadEntry, this);
			theThread[i].swap(aThread);
			while (lastTread == currThread); // wait for a little bit
		}
...
inline void Matrix2D<T, NR, NC>::threadEntry(Matrix2D<T, NR, NC>* thisPtr)
{
	...
	case 1:
		//currThread++; // Error had to comment out this line
