...
This is the cpp file of Shape2D with all it's functions and features.
Tasked feature: create the reccalcShape function to determine the shape's properties.
*/
...
float Shape2D::getProp(const std::string& propertyTag)
{
	// need some stuff here
	return 0.0f;
}

std::string Shape2D::outputProps()
{
	string output;
	// property tags are: perimeter, area, Ixx, Iyy, xBar, yBar
	vector<string> propList = { "perimeter", "area", "Ixx", "Iyy", "xBar", "yBar" /* , "futureProp"*/ };
	for (auto& curr : propList) {

		output += curr + ": " + StringPlus::sigFig(properties.at(curr), 6) + "\n";
	}

	return output;
}

void Shape2D::clearAllProperties()
{
	// go through all properties and set them to zero
	vector<string> propList = { "perimeter", "area", "Ixx", "Iyy", "xBar", "yBar" /* , "futureProp"*/ };
	for (auto& curr : propList) {
		properties[curr] = 0;
	}
...
void Shape2D::recalcShape()
{
	...
	if (thePoints.size() >= 3) { // not a valid shape
		Point2D prevPnt = thePoints.back();
		float widthxx, heightRectForX, heightTriForX, areaRectxx, areaTrixx, IxRect, IxTri, yBarRect, yBarTri;
		float widthyy, heightRectForY, heightTriForY, areaRectyy, areaTriyy, IyRect, IyTri, xBarRect, xBarTri;
		float sumAreaxx, sumAreaTimesYbar, sumIx, sumAreayy, sumAreaTimesXbar, sumIy;
		sumAreaxx = sumAreaTimesYbar = sumIx = sumAreayy = sumAreaTimesXbar = sumIy = 0;
		...
		for (auto currPnt : thePoints) {
			...
			// augment perimeter
			properties.at("perimeter") += Line2D::getLength(prevPnt, currPnt);
			

			// do the others . . .
			// removed some of the stuff we did in lecture in case it had errors

			// for XX axis
			widthxx = prevPnt.x - currPnt.x;
			heightRectForX = min(prevPnt.y, currPnt.y);
			heightTriForX = abs(prevPnt.y - currPnt.y);
			areaRectxx = widthxx * heightRectForX;
			areaTrixx = 0.5 * widthxx * heightTriForX;
			yBarRect = heightRectForX / 2;
			yBarTri = heightRectForX + (heightTriForX / 3);
			IxRect = (1.0 / 3.0) * widthxx * pow(heightRectForX, 3);
			IxTri = (1.0 / 36.0) * widthxx * pow(heightTriForX, 3) + areaTrixx * pow(yBarTri, 2);
			sumAreaxx += areaRectxx + areaTrixx;
			sumAreaTimesYbar += areaRectxx * yBarRect + areaTrixx * yBarTri;
			sumIx += IxRect + IxTri;

			// for YY axis
			widthyy = currPnt.y - prevPnt.y;
			heightRectForY = min(currPnt.x, prevPnt.x);
			heightTriForY = abs(currPnt.x - prevPnt.x);
			areaRectyy = widthyy * heightRectForY;
			areaTriyy = 0.5 * widthyy * heightTriForY;
			xBarRect = heightRectForY / 2;
			xBarTri = heightRectForY + (heightTriForY/ 3);
			IyRect = (1.0 / 3.0) * widthyy * pow(heightRectForY, 3);
			IyTri = (1.0 / 36.0) * widthyy * pow(heightTriForY, 3) + areaTriyy * pow(xBarTri, 2); ;
			sumAreayy += areaRectyy + areaTriyy;
			sumAreaTimesXbar += areaRectyy * xBarRect + areaTriyy * xBarTri;
			sumIy += IyRect + IyTri;
		...
		// removed some of the stuff we did in lecture in case it had errors
		properties["area"] = sumAreaxx;
		properties["xBar"] = sumAreaTimesXbar / sumAreayy;
		properties["yBar"] = sumAreaTimesYbar / sumAreaxx;
		properties["Ixx"] = sumIx - sumAreayy * properties["yBar"] * properties["yBar"];
		properties["Iyy"] = sumIy - sumAreaxx * properties["xBar"] * properties["xBar"];
	}
}
