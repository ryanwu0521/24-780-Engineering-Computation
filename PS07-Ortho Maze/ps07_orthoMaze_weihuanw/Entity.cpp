...
Tasked feature: find the maze's shortest path using Breadth First Search
*/
...
void Entity::findShortestPath() {
	...
	// start cell
	Cell currCell = { locRow, locCol };
	Cell neighborCell;
	// mark start cell as visited
	visited[locRow][locCol] = true;
	ProcessQueue.push(currCell);
	Cell endCell = myMaze->getEnd();
	int testRow, testCol;
	// while the end cell is not discovered and there are cells to process
	while (!ProcessQueue.empty()) {
		...
		for (int i = 0; i < 4; i++) {
			...
			// if the neighbor cell is within the maze boundaries, connected, and not visited
			if (testRow >= 0 && testRow < myMaze->getHeight() &&
				testCol >= 0 && testCol < myMaze->getWidth() &&
				myMaze->areConnected(currCell, { testRow, testCol }) &&
				!visited[testRow][testCol]) {
				// mark the neighbor cell as visited, set its parent, and add it to the queue
